...
Desktop release builds should create and populate `.venv` before running `npm run dist`; there is no separate build-venv npm script in this project. The virtual environment is platform-specific, so Windows, macOS, and Linux releases must be built separately on their target OS.
